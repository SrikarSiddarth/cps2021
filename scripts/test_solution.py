...
#        self.separation = 1.07
		...
		self.angle = Vector3()
		self.angular_vel = Vector3() 	# angular velocity fo the drone
		self.t = rospy.Time.now()
		self.probe_pos = [0]*3
		self.thrust = 0.7

		self.target_pos = [21,0]
		self.targetAngle = 0
		self.k = [0]*3
...
#	fixing the angle of throw to pi/8	
		...
		self.imu_sub = rospy.Subscriber('/mavros/imu/data',Imu,callback = self.imu_cb)
		...
		self.angle_pub = rospy.Publisher('/uav/angle',Vector3, queue_size=10)
...
#        self.test()
	...
	def imu_cb(self, msg):
		...
		o = euler_from_quaternion((msg.orientation.x,msg.orientation.y,msg.orientation.z,msg.orientation.w))
		self.angle.x = o[0]*180/pi
		self.angle.y = o[1]*180/pi
		self.angle.z = o[2]*180/pi
...
#	self.range = sqrt(self.target_pos[0]**2+self.target_pos[0]**2)
	...
	def pose_callback(self, msg):
		...
		self.angle.x = self.orientation[0]*180/pi
		self.angle.y = self.orientation[1]*180/pi
		self.angle.z = self.orientation[2]*180/pi
		self.angle_pub.publish(self.angle)
